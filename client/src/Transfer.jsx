...
function Transfer({  setBalance, privateKey }) {
  ...
  async function transfer(evt) {
    ...
    const [signature, recoveryBit] = await secp.sign(msgHash, hexToBytes(privateKey), {recovered: true})
    const signatureHex = toHex(signature)

    try {
      const {
        data: { balance },
      } = await server.post(`send`, {data: {msg, msgHash, signature: signatureHex, recoveryBit}});
      setBalance(balance);
    } catch (ex) {
